...
function CartBody({cartDisplayProducts}) {
  return cartDisplayProducts && cartDisplayProducts.map((cartItem, index) => <CartRow key={index} item={cartItem} />);
}
...
function CartRow({ item }) {
  return (
    <TR>
      ...
      <TD>{item.quantity}</TD>
      <TD>GBP {item.price} {item.extra > 0 && <><span><br/> +{item.extra}</span></>}</TD> 
      <TD>
...
function CartItemAttributes({attributes})
{
    ...
    return(
        attributes.length > 0 ?
            attributes.map((attr,index) => attr && <div className="flex flex-col space-y-2" key={`attr ${index}`}>
                ...
                <div>
                    ...
                    <span>
                        ...
                        {attr && attr.selected_options.map((item, index)=> item.subattr).join(", ") }
                    </span>
                </div>
            </div>
            )
...
export default function CartTable() {
    ...
    localapi
      ...
      .then((res) => {
        ...
        const displayCart = cart.map(cartItem => {
            ...
            const requiredProduct = res.data.products.filter(item => item.id == cartItem.product_id)
            ...
            if(cartItem.attributes.length > 0)
            {
                ...
                {
                    if(current)
                    return prev + current.selected_options.reduce((sum, curr) =>sum + curr.price, 0)
                }
                , 0)
            }
  ...
  useEffect(() =>{
    if(!cartDisplayProducts || cartDisplayProducts.length == 0)
    return 
    
    const subTotal = cartDisplayProducts.reduce((subtotal, curr) => subtotal + curr.price + (curr.extra || 0), 0 )
   
    setCartSubTotal(subTotal)
  }, [cartDisplayProducts])


  return <Table header={<CartHeader />} body={<CartBody cartDisplayProducts={cartDisplayProducts} />} footer={<CartFooter subTotal={cartSubTotal} />} />;
}
